  ...
  use wv_saturation,  only: qsat, qsat_o, qsat_wv, qsat_wv_gam, estbl, tmax, tmin, omeps, plenest
...
!!! start of initialization
   ...
   ! 3. Variables associated with cumulus convection
    ...
    integer     kk, mm, k, i, m, kp1, km1, ii
...
!   parameter (rkm = 16.0_r8)        !  Determine the amount of air that is involved in buoyancy-sorting [no unit]
    ...
    ! add for extend qsat
    real(r8) :: t_tmp(mix)     ! intermediate temperature for es(mix) look-up
    real(r8) :: weight(mix) ! Weight for interpolation
...
!!!end part init
    ...
    call t_startf("part_one_compute")
#ifdef SW_UWSHCU_01 param_uw_s1%ptr_param_uw1 = loc(param_uw_com_s1)
...
#endif
    call t_stopf("part_one_compute")

    call t_startf("part_one_out_put")
    do i = 1, iend
      if (id_exit(i) .or. id_exit_ex(i)) then
        cycle
      endif
...
33313   continue
    ...
    call t_stopf("part_one_out_put")

    call t_startf("part_one_compute2")
    do i = 1, iend
      if (id_exit(i) .or. id_exit_ex(i)) then
        cycle
      endif
...
3331   continue
    ...
    call t_stopf("part_one_compute2")
    do i = 1, iend
      if (id_exit(i) .or. id_exit_ex(i)) then
        cycle
      endif
...
#define QSAT_WV
#ifdef QSAT_WV
          t_tmp(i) = max(min(qsat_arg(i), tmax)-tmin, 0._r8)   ! Number of table entries above tmin
          ii = int(t_tmp(i)) + 1                     ! Corresponding index.
          weight(i) = t_tmp(i) - aint(t_tmp(i), r8)       ! Fractional part of t_tmp(i) (for interpolation).
          es(i) = (1._r8 - weight(i))*estbl(ii) + weight(i)*estbl(ii+1)


          ! If pressure is less than SVP, set qs(i) to maximum of 1.
          if ( (pe(i) - es(i)) <= 0._r8 ) then
             qs(i) = 1.0_r8
          else
             qs(i) = epsilo*es(i) / (pe(i) - omeps*es(i))
          end if

          ! Ensures returned es(i) is consistent with limiters on qs(i).
          es(i) = min(es(i), pe(i))
#else
...
!!!end part two
       ...
       do k = mkx, 1, -1  ! 'k' is a layer index : 'mkx'('1') is the top ('bottom') layer
          ...
          !call t_startf('qsat_o')
#ifdef QSAT_WV
          t_tmp(i) = max(min(t0(k, i), tmax)-tmin, 0._r8)   ! Number of table entries above tmin
          ii = int(t_tmp(i)) + 1                     ! Corresponding index.
          weight(i) = t_tmp(i) - aint(t_tmp(i), r8)       ! Fractional part of t_tmp(i) (for interpolation).
          es(i) = (1._r8 - weight(i))*estbl(ii) + weight(i)*estbl(ii+1)

          ! If pressure is less than SVP, set qs(i) to maximum of 1.
          if ( (p0(k, i) - es(i)) <= 0._r8 ) then
             qs(i) = 1.0_r8
          else
             qs(i) = epsilo*es(i) / (p0(k, i) - omeps*es(i))
          end if

          ! Ensures returned es(i) is consistent with limiters on qs(i).
          es(i) = min(es(i), p0(k, i))
#else
...
3333 if(id_exit(i)) then ! Exit without cumulus convection
...
#define CONDEN
  ...
  ! --------------------------------------------------------------------- !
    ...
    real(r8)              :: es_p              ! Saturation vapor pressure
    real(r8)              :: qs_p              ! Saturation spec. humidity
    ...
    t_tmp = max(min(temps, tmax)-tmin, 0._r8)   ! Number of table entries above tmin
    ii = int(t_tmp) + 1                     ! Corresponding index.
    weight = t_tmp - aint(t_tmp, r8)       ! Fractional part of t_tmp (for interpolation).
    es_p = (1._r8 - weight)*estbl(ii) + weight*estbl(ii+1)

    if ( (p - es_p) <= 0._r8 ) then
       qs_p = 1.0_r8
    else
       qs_p = epsilo*es_p / (p - omeps*es_p)
    end if
  ...
  ! ----------------------------------------------------------------- !
    ...
    !call t_startf('qsat_o')
#ifdef QSAT_WV
    t_tmp = max(min(Ti, tmax)-tmin, 0._r8)   ! Number of table entries above tmin
    ii = int(t_tmp) + 1                     ! Corresponding index.
    weight = t_tmp - aint(t_tmp, r8)       ! Fractional part of t_tmp (for interpolation).
    es = (1._r8 - weight)*estbl(ii) + weight*estbl(ii+1)

    ! If pressure is less than SVP, set qs to maximum of 1.
    if ( (psfc - es) <= 0._r8 ) then
       qs = 1.0_r8
    else
       qs = epsilo*es / (psfc - omeps*es)
    end if
