...
!#define CONDEN_EXPAND
!#ifdef CONDEN_EXPAND
!         call conden_expand(ps0(k-1),thl0bot,qt0bot,thj,qvj,qlj,qij,qse,id_check)
!#else
...
#define QSAT_WV
#ifdef QSAT_WV
          t_tmp = max(min(qsat_arg, tmax)-tmin, 0._r8)   ! Number of table entries above tmin
 ...
 45    continue
       ...
       do k = mkx, 1, -1  ! 'k' is a layer index : 'mkx'('1') is the top ('bottom') layer
          ...
          !call t_startf('qsat_o')
#ifdef QSAT_WV
          t_tmp = max(min(t0(k), tmax)-tmin, 0._r8)   ! Number of table entries above tmin
          ii = int(t_tmp) + 1                     ! Corresponding index.
          weight = t_tmp - aint(t_tmp, r8)       ! Fractional part of t_tmp (for interpolation).
          es = (1._r8 - weight)*estbl(ii) + weight*estbl(ii+1)
        
          ! If pressure is less than SVP, set qs to maximum of 1.
          if ( (p0(k) - es) <= 0._r8 ) then
             qs = 1.0_r8
          else
             qs = epsilo*es / (p0(k) - omeps*es)
          end if
        
          ! Ensures returned es is consistent with limiters on qs.
          es = min(es, p0(k))
#else
          call qsat_o(t0(k), p0(k), es, qs)          
#endif
          !call t_stopf('qsat_o')
